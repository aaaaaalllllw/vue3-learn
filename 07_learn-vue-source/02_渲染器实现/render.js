...
const patch = (n1, n2) => {
    ...
    //2.处理props
        ...
        for(const key in oldProps){
                ...
                if(key.startsWith("on")){
                    ...
                    el.removeListener(key.slice(2).toLowerCase(),value)
                }
                if(!(key in newProps)){
    ...
    //3.处理children
     ...
     if(typeof newChildren==='string'){//情况一：newChildren是一个字符串
       //边界情况(edge case)
       if(typeof oldChildren==='string'){
        ...
        if(typeof oldChildren==='string'){
           ...
           //3.newChildren.length<oldChild.length
           if(newChildren.length<oldChildren.length){
